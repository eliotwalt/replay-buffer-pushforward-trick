...
class ReplayBufferWrapper(torch.utils.data.Dataset):
    def __init__(
        ...
        if sampling_strategy == "uniform" and buffer_insertion_times is not None:
            raise ValueError("buffer_insertion_times should not be provided for uniform sampling")
    ...
    def init_main_process(
        ...
        buffer_insertion_times = manager.list() if sampling_strategy == "age_based" else None
        ...
        if sampling_strategy == "age_based":
            now = time.time()
    ...
    def add_to_buffer(self, index: int|torch.Tensor, sample: torch.Tensor, full_ok: bool=False, ignore_out_of_bounds: bool=False):
        ...
        with self.wlock:
            if len(self.buffer) >= self.buffer_size:
                ...
                if self.sampling_strategy == "age_based":
